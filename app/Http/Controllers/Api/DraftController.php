...
class DraftController extends BaseController
{
    ...
    public function saveQuotation(Request $request): JsonResponse
    {
        ...
        try {
			...
			$newArray=array();  
			$packageIds = [];
			//$this->pr($draftItems->toArray());die;
			foreach($draftItems->toArray() as $key => $val){
				$newKey=$val['package_id'];
				if(!array_key_exists($newKey,$newArray)){					
					$newArray[$newKey]['package_name'] = $val['package']['name'];
					$newArray[$newKey]['price'] = ($val['quantity'] * $val['price']);
					$newArray[$newKey]['items'] = [];										
				}else{
					$newArray[$newKey]['price'] += ($val['quantity'] * $val['price']);
				}
				
				
				if(!array_key_exists($val['device_id'],$newArray[$newKey]['items'])){
					
					$newArray[$newKey]['items'][$val['device_id']]['item_name'] = $val['device']['name'];
					$newArray[$newKey]['items'][$val['device_id']]['quantity'] =1;					
					
				}else{
					$newArray[$newKey]['items'][$val['device_id']]['quantity']++;
				}
			...
			//$this->pr($newArray);die;


            //$this->pr($setting['name']);die;
            //$this->pr($quotation->toArray());$this->pr($draftItems->toArray(),1);
            $pdf = PDF::loadView("quotation", ["quotation" => $quotation, "draft_items" => $draftItems, 'setting' => $setting,"user"=>$getUser,'new_item'=>$newArray]);
             ...
             $transport = (new SmtpTransport(config('mail.host_other'), config('mail.port_other'), config('mail.encryption_other')))->setUsername(config('mail.username_other'))->setPassword(config('mail.password_other'));
              ...
              Mail::send("quotation-email", ["quotationNo" => $quotationNo,"quotation" => $quotation, "draft_items" => $draftItems,"user"=>$getUser], function ($message) use ($pdf, $getUser, $quotationNo) {
                $message->to($getUser->email)
                    ...
                    ->from('sales@sentegrate.com.au', config('app.name'))
					->bcc([config('app.admin_email'),'syed.zaid@sentegrate.com.au'],config('app.name'))
                    ->attachData($pdf->output(), 'Sentegrate Quotation '.$quotationNo.'.pdf');
            });
            if (Mail::failures()) {
