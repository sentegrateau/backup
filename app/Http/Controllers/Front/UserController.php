...
class UserController extends Controller
{
    ...
    public $order_status = [0 => 'Pending payment', 1 => 'Initial payment received', 4 => 'On Hold', 2 => 'Dispatched', 3 => 'Completed', -1 => 'Cancelled', 5 => 'Refunded'  ,6=>'Incomplete Transaction'];
    ...
    public function myOrder(Request $request)
    {
        ...
        $orders =$orders->whereNotIn('order_status','6');
    ...
    public function orderstatus ( $order_id)
    {
        ...
        $recentOrderStatus = OrderStatusLog::where('order_id',$order->id)->orderBy('id','desc')->get();
        if($recentOrderStatus->count() > 0){
            $recentOrderStatus = $recentOrderStatus->push(['id' => count($recentOrderStatus)+1, 'status' => '0','created_at'=>$order->created_at,'updated_at'=>$order->updated_at,'remarks'=>'order is pending'])->toArray();
            sort($recentOrderStatus);
            $recentOrderStatus = $recentOrderStatus;
        }
        $total = $orderData['total'];
        $discount = $orderData['discount'];
        $paid_amt = $orderData['paid_amt'];
        $order_status = $this->order_status;
        return view('front.users.orderstatus', compact('order', 'shippingText', 'orders', 'total', 'order_id', 'discount', 'paid_amt','items','order_status','recentOrderStatus'));
	...
	public function invoiceview(Request $request, $order_id)
    {
        $orderData = $this->getOrderDetails($order_id);
        $order = $orderData['order'];
        $items = $this->filteringSubItems($order->toArray());
        ...
        return view('invoice', compact('order', 'shippingText', 'orders', 'total', 'order_id', 'discount', 'paid_amt','items','quotation'));
    ...
    public function orderThankYou(Request $request, $order_id)
    {
        ...
        if($userData->user_referred_site!='sentegrate'){
            //$url = "https://sentegrate.com/invalid-token";
            $url = "https://www.digitalhomesystems.com.au";
        }else{
            $url = "http://sentegrate.com.au";
        }
        ...
        $discount = $orderData['discount'];
        $paid_amt = $orderData['paid_amt'];
        return view('front.users.thank-you', compact('url','order', 'shippingText', 'orders', 'total', 'order_id', 'discount', 'paid_amt','items'));
    }
