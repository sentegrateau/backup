            ...
            <div class="box">
                <div class="box-header with-border">
                    <button style="margin-bottom: 10px" class="btn btn-primary delete_all"
                            data-url="{{ route('page.deleteAll') }}">Delete All Selected
                    </button>
                    ...
                    <table class="table table-bordered">
                        ...
                        @foreach ($page as $blg)
                            <tr id="tr_{{$blg->id}}">
                                ...
                                <td>{{$blg->name}}</td>
                                <td>{{ucfirst(str_replace('-',' ',$blg->type))}}</td>
                                <td>{{substr(strip_tags($blg->description),0,20)}}</td>
                                <td>{{($blg->status)?'Active':'Deactive'}}</td>
                                <td>
                                    <a href="{{route('page.edit',$blg->id)}}">
                                        <i class="fa fa-pencil" aria-hidden="true"></i>
                                    </a>
                                    <a href="{{route('page.delete',$blg->id)}}">
