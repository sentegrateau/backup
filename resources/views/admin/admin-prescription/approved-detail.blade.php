                        ...
                        <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
                            @if(!empty($prescription->prescriptionRelation))
                                @foreach($prescription->prescriptionRelation as $key=>$value)
                                    @php
                                        $totalAmount = array_sum(array_column($value->medicineOrder->toArray(),'price'));
                                    @endphp
                                    <div class="panel panel-default">
                                        <div class="panel-heading {{$value->status == 1?'bg-green':'bg-red'}}" role="tab" id="heading{{$key+1}}">
                                        ...
                                        <div id="collapse{{$key+1}}" class="panel-collapse collapse {{$value->status == 1?'in':''}}" role="tabpane{{$key+1}}" aria-labelledby="heading{{$key+1}}">
                                            <div class="panel-body">
                                                @if(!empty($value->medicineOrder))
                                                    <table class="table table-striped">
                                                        ...
                                                        <tbody>
                                                            @foreach($value->medicineOrder as $keyM=>$valueM)
                                                                <tr>
...
<style>
    .panel-group .panel-heading a {padding:5px 5px;display: block;text-decoration: none;position: relative;}
    ...
    .panel-title a:hover, .panel-title a:active, .panel-title a:focus{color:#fff;}
    .bg-red{background-color:#dd4b39b5!important;}
    .bg-green{background-color:#00a65aab!important;}
</style>
